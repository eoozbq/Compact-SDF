...
def decode_sdf(decoder, latent_vector, queries):
    ...
    try:
        ...
        print(e)
        latent_repeat = latent_vector.expand(num_samples, -1)
        sdf, _ = decoder.forward(latent_vecs=latent_repeat.cuda(), xyz_l=queries.cuda())

    return sdf


def gl_sdf(decoder, latent_vector, queries, low_tag=False):
    num_samples = queries.shape[0]
    latent_repeat = latent_vector.expand(num_samples, -1)
    low_sdf, high_sdf, _ = decoder(latent_repeat.cuda(), queries.cuda(), queries.cuda())
    sdf = high_sdf
    return sdf
