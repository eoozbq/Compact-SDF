...
def mesh_out(decoder, N, max_batch, latent_vec, filename, offset, scale):
    ...
    if 'gen' in os.path.basename(filename):
        low_tag = True
    else:
        low_tag = False
...
def create_mesh(
    ...
    tree = get_tree()
    decoder.eval()
    gen_out, sdf_values = mesh_out(decoder, N, max_batch, latent_vec, ply_filename + '-gen' + ".ply", offset,
                                       scale)
    ...
    samples_points = gen_out[:, :3]
    samples_points_sdf = gen_out[:, 3]
