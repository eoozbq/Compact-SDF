...
def evaluate(experiment_directory, checkpoint, gt_objpath='', deepsdf_mesh=None):
    chamfer_results = []
    num_mesh_samples = 100000
    instance_name = deepsdf_mesh
    logging.debug(
    ...
    chamfer_deepsdf_dist = deep_sdf.metrics.chamfer.compute_trimesh_chamfer(
        ground_truth_points,
        reconstruction,
        0,
        1,
    ...
    reconstruction = trimesh.load(reconstructed_mesh_filename.replace('-deepsdf', '-cube_code'))
    ...
    reconstruction = trimesh.load(reconstructed_mesh_filename.replace('-deepsdf.ply', '.ply'))
    ...
    chamfer_results.append(
        (
            os.path.join(instance_name.split('-')[0]), chamfer_deepsdf_dist, chamfer_cubesdf_dist,
            chamfer_combine_dist)
    )
    print(chamfer_deepsdf_dist, chamfer_cubesdf_dist, chamfer_combine_dist, instance_name)
...
if __name__ == "__main__":
    ...
    for mesh in recon_mesh_all:
        if 'deepsdf' and 'cube' not in mesh:
    ...
    def parallel_evaluate(recon_mesh):
        chamfer_results, cd_path = evaluate(
            ...
            deepsdf_mesh=recon_mesh
        )
        return chamfer_results, cd_path


    with Pool(processes=100) as p:
        chamfer_results = p.map(parallel_evaluate, deepsdf_mesh)
