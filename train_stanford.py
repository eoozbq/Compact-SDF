...
def main_function(experiment_directory, continue_from):
    ...
    decoder = arch.combined_model(latent_size, world_size=128).cuda()
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for gen_data, cube_data, indices in sdf_loader:

            # Process the input data
            gen_data = gen_data.reshape(-1, 4)
            cube_data = cube_data.reshape(-1, 4)

            num_global_sdf_samples = gen_data.shape[0]
            num_cube_samples = cube_data.shape[0]

            gen_data.requires_grad = False
            cube_data.requires_grad = False

            xyz_g = gen_data[:, 0:3].to(device, non_blocking=True)
            xyz_l = cube_data[:, 0:3].to(device, non_blocking=True)
            sdf_g_gt = gen_data[:, 3].unsqueeze(1).to(device, non_blocking=True)
            ...
            formatted_output = f"Chunk Loss: {chunk_loss:.10f}, gen_sdf_loss: {sdf_g_loss:.10f}, cube_sdf_loss: {sdf_l_loss:.10f}, Reg Loss: {reg_loss:.10f}, Reg cube Loss: {reg_loss_cube:.10f}"
